...
CREATE TABLE imap_messages (ROWID INTEGER PRIMARY KEY AUTOINCREMENT, message INTEGER REFERENCES messages(ROWID) ON DELETE SET NULL, mailbox INTEGER REFERENCES mailboxes(ROWID) ON DELETE CASCADE, flags INTEGER, uid INTEGER);
...
CREATE INDEX action_ews_messages_action_id_index ON action_imap_messages(action);
CREATE INDEX ews_copy_action_messages_action_id_index ON imap_copy_action_messages(action);
...
CREATE INDEX action_imap_messages_action_index ON imap_copy_action_messages(destination_message);
CREATE INDEX ews_copy_action_messages_action_in, unread_count), unread_count_adjusted_for_duplicates - 1) WHERE ((mailboxes.ROWID = OLD.mailbox AND mailboxes.source ISNULL AND OLD.message_id ISNULL) OR (mailboxes.source NOTNULL AND mailboxes.ROWID IN (SELECT mailbox_id FROM labels WHERE message_id = OLD.ROWID) AND ((mailboxes.source = OLD.mailbox AND OLD.message_id ISNULL) OR ((SELECT count() FROM labels WHERE message_id IN (SELECT ROWID FROM messages WHERE message_id = OLD.message_id) AND mailbox_id = mailboxes.ROWID) = 0 AND OLD.message_id NOTNULL)))) AND (OLD.fs.ROWID = OLD.mailbox AND mailboxes.source ISNULL) OR (mailboxes.ROWID IN (SELECT mailbox_id FROM labels WHERE message_id = OLD.ROWID) AND mailboxes.source = OLD.mailbox);
...
UPDATE mailboxes SET deleted_count = deleted_count + 1 WHERE ((ma_E mails  deleted_count + 1 WHERE mailboxes.ROWID = NEW.mailbox_id AND mailboxes.source IN (SELECT mailbox FROM messages WHERE ROWID = NEW.message_id AND flags&2 != 0 LIMIT 1);
...
UPDATE mailboxes SET unread_count_adjusted_for_duplicates = MIN(unread_count_a WHERE mailboxes.ROWID = OLD.mailbox_id AND mailboxes.source IN (SELECT mailbox FROM messages WHERE ROWID = OLD.message_id LIMIT 1);
UPDATE mailboxes SET unseen_count = MAX(0, unseen_count - 1) WHERE mailboxes.ROWID =.message_id) AND mailbox_id = OLD.mailbox_id) = 0;
